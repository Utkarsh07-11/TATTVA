...
import indianUndergroundHaulage from '../assets/indian_mines/underground-stope-cavern.jpg';
    ...
    <div className="space-y-14 pb-20 max-w-7xl mx-auto px-4 sm:px-6 pt-6">
      ...
      <div className="border-b border-[#DCD5CD] pb-6">
        <div className="text-xs font-sans tracking-wider text-[#C87A5B] uppercase font-semibold mb-2 flex items-center gap-2 text-slide-down">
          <TrendingUp className="w-3.5 h-3.5 text-[#C87A5B]" />
      ...
      {/* 2. Formal Warm Linen Section: The Domain Bridge Hierarchy */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div className="lg:col-span-7 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 font-sans text-xs">
              <div className="p-3 bg-white border border-[#DCD5CD] rounded-sm shadow-xs">
                <span className="text-[#8A817D] block text-[11px]">CORPORATE ANNUAL</span>
                <span className="font-bold text-[#26211F] text-sm">1.75M Tonnes</span>
                <span className="text-[#8A817D] block text-[10px]">Total Production</span>
              </div>
              <div className="p-3 bg-white border border-[#DCD5CD] rounded-sm shadow-xs">
                <span className="text-[#8A817D] block text-[11px]">REGIONAL SHARE</span>
                <span className="font-bold text-[#C87A5B] text-sm">~670,000 Tonnes</span>
                <span className="text-[#8A817D] block text-[10px]">38.4% Output</span>
              </div>
              <div className="p-3 bg-white border border-[#DCD5CD] rounded-sm shadow-xs">
                <span className="text-[#8A817D] block text-[11px]">PIT TARGET</span>
                <span className="font-bold text-emerald-700 text-sm">10,000 t / mo</span>
                <span className="text-[#8A817D] block text-[10px]">Active Shift Stope</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5">
            <BlockRevealImage
              src={indianUndergroundHaulage}
              alt="Illuminated underground mining stope crosscut and haulage level"
      ...
      {backtest && (
        <section id="tour-backtest-section" className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              <span className="text-xs font-sans text-[#C87A5B] uppercase tracking-wider font-semibold">
                STATUTORY PRODUCTION BACKTEST
              </span>
              <h3 className="font-sans text-2xl text-[#26211F] font-bold">
              ...
              return (
                <div
                  ...
                  className={`story-card p-5 relative ${
                    isWinner ? 'border-[#C87A5B] bg-[#FAF7F2]' : ''
                  }`}
                >
                  {isWinner && (
                    <div className="absolute top-3 right-3 flex items-center gap-1 text-[10px] font-sans font-bold text-[#C87A5B] bg-[#EDC7B7]/30 px-2 py-0.5 rounded-full border border-[#EDC7B7]">
                  ...
                  <div className="mt-2 text-2xl sm:text-3xl font-sans font-bold text-[#26211F] tracking-tight">
                    {metrics.mape_pct}%{' '}
                    <span className="text-xs font-normal text-[#8A817D] font-sans">MAPE</span>
                  </div>
                  <div className="mt-2 text-xs font-sans text-[#5A524F] space-y-1">
          ...
          <div className="story-card p-6 border border-[#DCD5CD]">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
              ...
              <div className="text-xs font-sans text-[#5A524F] flex items-center gap-2">
                <span className="w-2.5 h-2.5 rounded-full bg-[#C87A5B]" /> Audited Actual
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-600" /> Trend Projection
                <span className="w-2.5 h-2.5 rounded-full bg-[#8A817D]" /> Prior Year Baseline
              </div>
            </div>
            ...
            <div className="h-80 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={chartData} margin={{ top: 10, right: 20, left: 10, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#DCD5CD" opacity={0.7} />
                  <XAxis dataKey="year" stroke="#8A817D" fontSize={11} />
                  <YAxis
                    stroke="#8A817D"
                    fontSize={11}
          ...
          <div className="story-card p-6 overflow-x-auto">
            <h4 className="font-sans text-lg text-[#26211F] font-bold mb-3">
              Annual Statutory Backtest Audit Log
            </h4>
            <table className="w-full text-xs font-sans text-left">
              <thead className="text-[#8A817D] border-b border-[#DCD5CD] uppercase tracking-wider text-[10px]">
                <tr>
              ...
              <tbody className="divide-y divide-[#DCD5CD]">
                {backtest.folds.map((fold) => (
                  <tr key={fold.fiscal_year} className="hover:bg-[#FAF7F2] transition-colors">
                    <td className="py-2.5 pr-4 font-bold text-[#26211F]">{fold.fiscal_year}</td>
                    <td className="py-2.5 pr-4 text-[#C87A5B] font-semibold">{fmt(fold.actual_mt)} t</td>
                    <td className="py-2.5 pr-4 text-[#5A524F]">{fmt(fold.naive_last_year_mt)} t</td>
                    <td className="py-2.5 pr-4 text-[#5A524F]">{fmt(fold.moving_average_3y_mt)} t</td>
                    <td className="py-2.5 pr-4 text-emerald-700 font-semibold">{fmt(fold.ridge_trend_mt)} t</td>
                    <td
                      className={`py-2.5 font-bold ${
                        fold.ridge_error_mt > 0 ? 'text-[#C87A5B]' : 'text-emerald-700'
                      }`}
                    >
      ...
      <section id="tour-daily-analytics" className="border-t border-[#DCD5CD] pt-10 space-y-6">
        <div>
          <div className="text-xs font-sans tracking-wider text-[#C87A5B] uppercase font-semibold mb-1">
            DAILY EXTRACTION TELEMETRY
          </div>
          <h3 className="story-headline text-2xl text-[#26211F] font-bold mt-1">
            Stope Extraction Run-Rate and Daily Production Analytics
          </h3>
          <p className="text-xs text-[#5A524F] font-sans mt-1 max-w-2xl">
